...
public class WebSocket : NSObject, NSStreamDelegate {
    ...
    private func initStreamsWithData(data: NSData, _ port: Int) {
        ...
        if self.selfSignedSSL {
            ...
            self.inputStream!.setProperty(settings, forKey: String(kCFStreamPropertySSLSettings))
            self.outputStream!.setProperty(settings, forKey: String(kCFStreamPropertySSLSettings))
        }
        isRunLoop = true
        ...
        if response.isFin && response.bytesLeft <= 0 {
            if response.code == .Ping {
                ...
                dispatch_async(queue,{
                    ...
                    self.delegate!.websocketDidReceiveMessage(self, text: String(str!))
                })
